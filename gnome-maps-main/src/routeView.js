...
import {CPPView} from './cppView.js';
...
const ID_COVERAGE = 'coverage';

export class RouteView extends Gtk.Box {
    ...
    _initTransportationToggles() {
        ...
        this._modeChooser.connect('notify::active', () => {
            ...
            } else if (activeModeId === ID_COVERAGE) {
                mode = transport.COVERAGE;
            }
        ...
        let setToggles = function() {
            ...
            case transport.TRANSIT:
                ...
                break;
            case transport.COVERAGE:
                activeModeId = ID_COVERAGE;
                break;
            }
    ...
    _switchRoutingMode(mode) {
        if (mode === RouteQuery.Transportation.COVERAGE) {
            this._mainStack.visible_child_name = 'coverage';
            if (!this._cppView) {
                this._cppView = new CPPView({ mapView: this._mapView });
                this._cppViewContainer.append(this._cppView);
            }
        } else {
            this._mainStack.visible_child_name = 'standard';
...
GObject.registerClass({
    Template: 'resource:///org/gnome/Maps/ui/route-view.ui',
    InternalChildren: [ 'mainStack',
                        'cppViewContainer',
                        'distanceInfo',
                        'entryList',
                        'instructionList',
