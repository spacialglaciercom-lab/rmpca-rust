...
// CPP (Chinese Postman) route colors
const CPP_ROUTE_COLOR =
    new Gdk.RGBA({ red:   0xff / 255,
                   green: 0x78 / 255,
                   blue:  0x00 / 255,
                   alpha: 1.0 }); // #ff7800
const CPP_ROUTE_OUTLINE_COLOR =
    new Gdk.RGBA({ red:   0xcc / 255,
                   green: 0x5f / 255,
                   blue:  0x00 / 255,
                   alpha: 1.0 }); // #cc5f00
...
export class MapView extends Gtk.Overlay {
    ...
    showTransitStop(transitStop, transitLeg) {
        ...
        this._turnPointMarker = new TurnPointMarker({ transitStop: transitStop,
                                                      ...
                                                      mapView: this });
        this._instructionMarkerLayer.add_marker(this._turnPointMarker);
        this._mainWindow.showPlace(transitStop,
    ...
    /* ---- CPP (Chinese Postman) area-coverage route methods --------------- */

    /**
     * Enable bounding-box selection mode on the map.
     * The user can click-drag to draw a rectangle. When they release,
     * @callback is called with the resulting BoundingBox.
     */
    enableBBoxSelection(callback) {
        this._bboxSelector.enable(callback);
    }

    /**
     * Disable bounding-box selection mode and remove the overlay.
     */
    ...
    /**
     * Show a CPP (area-coverage) route on the map.
     * Uses orange color (#ff7800) to distinguish from turn-by-turn routes.
     *
    ...
    showCPPRoute(cppRoute) {
        ...
        cppRoute.path.forEach((point) => {
            let marker = new Shumate.Marker({
                latitude:  point.latitude,
                longitude: point.longitude,
                visible:   false,
            });
            routeLayer.add_node(marker);
        });

        this._cppRouteLayers.push(routeLayer);
        this.routingOpen = true;

        if (cppRoute.bbox && cppRoute.bbox.isValid())
            this.gotoBBox(cppRoute.bbox);
    }
